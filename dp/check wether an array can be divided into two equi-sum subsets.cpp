...
int n;
vi v(1000);
bool recurse(int index,int sum,int total)
{
    if(index==n-1)
        return 2*sum==total || 2*(sum+v[index])==total;
    return recurse(index+1,sum,total) || recurse(index+1,sum+v[index],total);
}
int32_t main()
{
    ...
    cin>>n;
    input(v,n);
    cout<<recurse(0,0,accumulate(v.begin(),v.begin()+n,0))<<endl;
}

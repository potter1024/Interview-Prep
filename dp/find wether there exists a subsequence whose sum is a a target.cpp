...
int n;
vi v(1000);
bool recurse(int index,int sum,int target)
{
    if(index==n-1)
        return sum+v[index]==target || sum==target;
    return recurse(index+1,sum,target) || recurse(index+1,sum+v[index],target);
}
int32_t main()
{

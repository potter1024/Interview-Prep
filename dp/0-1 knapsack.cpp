...
int n,capacity;
vi weight(1000),value(1000);
int knapsack(int index,int weight_taken,int value_taken)
{
    if(index==n-1)
    {
        if(weight_taken+weight[index]>capacity)
            return value_taken;
        return value_taken+value[index];
    }
    if(weight[index]+weight_taken>capacity)
        return knapsack(index+1,weight_taken,value_taken);
    return max(knapsack(index+1,weight_taken+weight[index],value_taken+value[index]),knapsack(index+1,weight_taken,value_taken));
}
int32_t main()
{
